...
clanInfoPath = Path(__file__).parent.joinpath("./clanInfo/clanInfo.json")
...
def state_image_generate(groupBossData: dict, bossStateImageList: list, clanInfo: dict, groupId: Optional[int] = 0, userId: Optional[int] = 0) -> Image.Image:
    global save_picture_num

    groupId = str(groupId)
    with open(clanInfoPath, 'r', encoding='utf-8') as f:
        file_data = json.load(f)
    if file_data.get(groupId) and file_data[groupId].get("rank"):
        clanRank = file_data[groupId]["rank"]
    else:
        clanRank = 114514
    ...
    for i in range(1, 12):
        clanSession = str(5000 + i)
        if groupBossData[5]["name"] == data[clanSession]["cnName"]:
            break
        elif groupBossData[5]["name"] == data[clanSession]["jpName"]:
            break
    # 生成背景图片
    resultImage = Image.new("RGBA", (2048, 1536), (0, 0, 0, 0))
    bgClanBattle = Image.open(os.path.join(texturePath, "background", f"{clanSession}.png"))  # 已关闭
    ...
    for bossNum in range(1, 6):
        ...
        for i in range(1, 18):
            ...
            if thisBossData["name"] == data[bossId]["cnName"]:
                ...
                break
            elif thisBossData["name"] == data[bossId]["jpName"]:
                actualBossId.append(bossId)
                monsterIcon.append(
                    monster_icon_generate(bossId, thisBossData["health"], thisBossData["full_health"],
                                          thisBossData["cycle"]))
    ...
    # 排名绘制
    rankImage = rank_num_generate(clanRank, clanInfo["selfRank"])
    resultImage.alpha_composite(rankImage, (1060, 103))

    # 日期绘制
    # 懒得去做封装了，能用就行，直接复制粘贴
    if file_data.get("database") and file_data["database"].get("start_time") and file_data["database"].get("end_time"):
        clanBattleNum = Image.open(os.path.join(texturePath, "clanBattleNum.png")) # 已关闭
        numPasteWidth = 50
        for string in file_data["database"]["start_time"]:
            if string == "1":
                clanBattleTime = clanBattleNum.crop((7, 0, 32, 33))
                numPasteWidth -= 7
                resultImage.alpha_composite(clanBattleTime, (numPasteWidth, 180))
                numPasteWidth += 18
            elif string in {'0', '2', '3', '4', '5', '6', '7', '8', '9'}:
                strNum = int(string)
                if strNum == 0:
                    clanBattleTime = clanBattleNum.crop((288, 0, 320, 33))
                    numPasteWidth -= 7
                    resultImage.alpha_composite(clanBattleTime, (numPasteWidth, 180))
                    numPasteWidth += 25
                else:
                    clanBattleTime = clanBattleNum.crop((((strNum - 1) * 32), 0, (strNum * 32), 33))
                    numPasteWidth -= 7
                    resultImage.alpha_composite(clanBattleTime, (numPasteWidth, 180))
                    numPasteWidth += 25
            elif string == "/":
                clanBattleTime = clanBattleNum.crop((359, 0, 387, 33))
                numPasteWidth -= 7
                resultImage.alpha_composite(clanBattleTime, (numPasteWidth, 180))
                numPasteWidth += 21
            elif string == ":":
                clanBattleTime = clanBattleNum.crop((387, 0, 408, 33))
                numPasteWidth -= 7
                resultImage.alpha_composite(clanBattleTime, (numPasteWidth, 180))
                numPasteWidth += 15
            elif string == " ":
                ...
                pass
        clanBattleTime = clanBattleNum.crop((320, 0, 357, 33))
        numPasteWidth += 7
        resultImage.alpha_composite(clanBattleTime, (numPasteWidth, 180))
        numPasteWidth += 44
        for string in file_data["database"]["end_time"]:
            if string == "1":
                clanBattleTime = clanBattleNum.crop((7, 0, 32, 33))
                numPasteWidth -= 7
                resultImage.alpha_composite(clanBattleTime, (numPasteWidth, 180))
